...
Trees in order of falling weight:
0.5 threshold: 30 8 31 10 2 21 6 9 4 24 14 29 16 26 19 32 20 13 1 23 25 7 12 35 28 33 27 5 17 11 18 22 3 34 15
Best model has 6 trees - trees: 30 8 31 10 2 21

0.9 threshold: 5 33 25 17 24  9 22  2 15  4 32 35 10
Best model has 4 trees: 5 33 25 17

0.1 threshold: 28 22 27 13 9 8 16 23 7 26
Best model has 3 trees: 28 22 27 
...
```{r}
BIC_table <- read.csv(paste0("../output/", threshold, "/bic_values.csv"), header = TRUE)
...
```{r}
path_to_trees <- "../output/0.1/mast_model_6/mast_model_final_6.treefile"
...
#tree_nrs <- c(5, 33, 25, 17, 24)
tree_nrs <- c(28, 22, 27, 13, 9, 8)
...
tree10_scf <- read.tree("../output/0.5_output/scf/scf_tree_10.cf.tree")
...
aln <- readDNAMultipleAlignment(paste0("../data/", threshold, "_trimmed_alns_HIV_2021_subset.fasta"))
...
dna <- read.dna(paste0("../data/", threshold, "_trimmed_alns_HIV_2021_subset.fasta"), format = "fasta")
M <- as.character(dna)
...
```{r}
ggsave("../figures/0.9_gappy_entropy.png", comb_plot, device = 'png', width = 190, height = 70, units = 'mm', dpi = 300, bg = 'white')
```
...
### Site likelihood - we DO use these 
